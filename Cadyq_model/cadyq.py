...
from Quant_ops.quant_ops import quant_act_pams
...
class BitSelector(nn.Module):
    def __init__(self, n_feats, bias=False, ema_epoch=1, search_space=[4, 6, 8], linq=False):
        ...
        self.net_small = nn.Sequential(nn.Linear(n_feats + 2, len(search_space)))
    ...
    def forward(self, x):
        ...
        if len(self.search_space) == 3:
            ...
            out_soft = p1.view(p1.size(0), 1, 1, 1) * q_bit1 + p2.view(p2.size(0), 1, 1, 1) * q_bit2 + p3.view(
                p3.size(0), 1, 1, 1) * q_bit3
            out_hard = (flag == 0).view(flag.size(0), 1, 1, 1) * q_bit1 + (flag == 1).view(flag.size(0), 1, 1,
                                                                                           1) * q_bit2 + (
                                   flag == 2).view(flag.size(0), 1, 1, 1) * q_bit3
